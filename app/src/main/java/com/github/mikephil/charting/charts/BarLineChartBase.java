...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    protected float getMinVisibleLimitLine ( YAxis AxisVal , float min_value )
    {

        List<LimitLine> limitLines = AxisVal.getLimitLines();
        ...
        for (int i = 0; i < limitLines.size(); i++)
        {
            ...
            if ( l.getLimit() < min_value )
                min_value = l.getLimit()  ;
    ...
    protected float getMaxVisibleLimitLine ( YAxis AxisVal , float max_value )
    {
        List<LimitLine> limitLines = AxisVal.getLimitLines();
        ...
        for (int i = 0; i < limitLines.size(); i++)
        {
            ...
            if ( l.getLimit() > max_value )
                max_value = l.getLimit()  ;
    ...
    protected void autoScale() {
        ...
        if (mAxisLeft.isEnabled())
        {
            float Calc_Values = mData.getYMax(AxisDependency.LEFT) ;

            float Max_Calc = getMaxVisibleLimitLine( mAxisLeft , Calc_Values );

            Calc_Values = mData.getYMin(AxisDependency.LEFT) ;
            float Min_Calc = getMinVisibleLimitLine( mAxisLeft , Calc_Values );

            mAxisLeft.calculate( Min_Calc , Max_Calc ) ;
        }

        if (mAxisRight.isEnabled())
        {
            float Calc_Values = mData.getYMax(AxisDependency.RIGHT) ;

            float Max_Calc = getMaxVisibleLimitLine( mAxisRight , Calc_Values );

            Calc_Values = mData.getYMin(AxisDependency.RIGHT) ;
            float Min_Calc = getMinVisibleLimitLine( mAxisRight , Calc_Values );

            mAxisRight.calculate( Min_Calc , Max_Calc ) ;
