...
public abstract class DataSet<T extends Entry> extends BaseDataSet<T> {
    ...
    public void calcMinMaxY(float fromX, float toX) {
        ...
        for (int i = indexFrom; i <= indexTo; i++) {
            ...
            // only recalculate y
            calcMinMaxY(getEntryForIndex(i));
        }
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < mValues.size(); i++) {
            buffer.append(getEntryForIndex(i).toString() + " ");
        }
    ...
    public void addEntryOrdered(T e) {
        ...
        if (mValues.size() > 0 && getEntryForIndex(mValues.size() - 1).getX() > e.getX()) {
    ...
    public int getEntryIndex(float xValue, float closestToY, Rounding rounding) {
        ...
        while (low < high) {
            ...
            final float d1 = getEntryForIndex(m).getX() - xValue,
                    d2 = getEntryForIndex(m + 1).getX() - xValue,
        ...
        if (closest != -1) {
            float closestXValue =getEntryForIndex(closest).getX();
            ...
            if (!Float.isNaN(closestToY)) {
                while (closest > 0 && getEntryForIndex(closest - 1).getX() == closestXValue)
                    closest -= 1;

                float closestYValue = getEntryForIndex(closest).getY();
                ...
                while (true) {
                    ...
                    final Entry value = getEntryForIndex(closest);
    ...
    public List<T> getEntriesForXValue(float xValue) {
        ...
        while (low <= high) {
            int m = (high + low) / 2;
            T entry = getEntryForIndex(m);

            // if we have a match
            if (xValue == entry.getX()) {
                while (m > 0 && getEntryForIndex(m - 1).getX() == xValue)
                    m--;
